...
export class $Set extends Set {
    ...
    #closeTransaction() {
        ...
        const increment = difference(this.#transactionIncrement, this);
        this.#transactionIncrement.clear();
        for (const value of increment)
            super.add(value);
        const decrement = intersection(this.#transactionDecrement, this);
        this.#transactionDecrement.clear();
        for (const value of decrement)
            super.delete(value);
     ...
     */
    _openTransaction() {
        if (!this.#isReactivityEnabled)
            throw new ReactivityDisabledError(`${this} is immutable!`);
    ...
    #batchReactiveAdd(values) {
        if (this.#transactionIsOpen) {
            ...
            if (values instanceof Set) {
                const increment = difference(values, this);
                for (const value of increment)
                    super.add(value);
                if (increment.size)
                    this.onChange.dispatchSignal({ increment, decrement: undefined });
            }
            else {
    ...
    #reactiveDelete(value) {
        ...
        if (this.#transactionIsOpen) {
            ...
            if (super.delete(value))
                this.onChange.dispatchSignal({ increment: undefined, decrement: new Set([value]) });
        }
        return true;
    }
    /**
    ...
    #batchReactiveDelete(values) {
        if (this.#transactionIsOpen) {
            ...
            if (values instanceof Set) {
                const decrement = intersection(this, values);
                for (const value of decrement)
                    super.delete(value);
                if (decrement.size)
                    this.onChange.dispatchSignal({ increment: undefined, decrement });
            }
            else {
    ...
    #reactiveOverwrite(values) {
        if (this.#transactionIsOpen) {
            ...
            if (values instanceof Set) {
                const increment = difference(values, this);
                const decrement = difference(this, values);
                for (const value of increment)
                    super.add(value);
                for (const value of decrement)
                    super.delete(value);
     ...
     * @private
     */
    #reactiveApplyChanges(changes) {
        if (!changes)
            return;
        if ("overwrite" in changes) {
            if (changes.overwrite)
                this.#reactiveOverwrite(changes.overwrite);
        }
        else if (changes.increment || changes.decrement) {
            this.#openTransaction();
            if (changes.increment)
                this.#batchReactiveAdd(changes.increment);
            if (changes.decrement)
                this.#batchReactiveDelete(changes.decrement);
